...
if i == numberOfMeassurements
    ...
    for lidarChannel = 1:8
        ...
        histogram(lidar_measurements(:, lidarChannel), 90);
        xlabel('Meassurements'); ylabel('Count');
        title(sprintf('Channel %d', lidarChannel));
    end

    figure('Name','Gnss histograms');
    for gnssChannel = 1:2
        subplot(1, 2, gnssChannel );
        histogram(gnss_measurements(:, gnssChannel ), 90);
        title(sprintf('Channel %d', gnssChannel));
    end
...
if i == numberOfMeassurements
    ...
    mainDiagonalLidar = diag(lidar_covariance)
    mainDiagonalGnaa = diag(gnss_covariance)
...
%% Task 4
if i == numberOfMeassurements
    x_lidar = linspace(-4*lidarChannelsStandartDev(1), 4*lidarChannelsStandartDev(1), 1000);
    x_gnss = linspace(-4*gnssChannelsStandartDev(1), 4*gnssChannelsStandartDev(1), 1000);

    lidarNormPDF = norm_pdf(x_lidar, 0, lidarChannelsStandartDev(1));
    GNSSNormPDF = norm_pdf(x_gnss, 0, gnssChannelsStandartDev(1));

    figure('Name','Probability density functions')
    subplot(1, 2, 1);
    
    plot(x_lidar, lidarNormPDF)

    subplot(1, 2, 2);
    plot(x_gnss, GNSSNormPDF)
end
